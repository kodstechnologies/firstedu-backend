 ...
 *   Competitive > Government > SSC CGL Tier 2
 * Idempotent. Keeps Tier 1 + UPSC siblings.
 */
...
const run = async () => {
  ...
  if (upsc && (upsc.order == null || upsc.order < 2)) {
    await Category.updateOne({ _id: upsc._id }, { $set: { order: 2 } });
  }
    ...
    JSON.stringify(
      {
        ok: true,
        exam: {
          path: "Competitive > Government > SSC CGL Tier 2",
          id: String(ssc._id),
          created,
        },
      },
...
run().catch(async (err) => {
  console.error("seed-ssc-cgl-tier2-exam-categories failed:", err);
  try {
    await mongoose.connection.close();
  } catch {
    /* ignore */
