...
const papers = await GmatCompetitivePaper.find()
  .select("paperKey title totalQuestions sourceFile")
  .sort({ sortOrder: 1 })
  .lean();
const total = await GmatCompetitiveQuestion.countDocuments();
const missing = await GmatCompetitiveQuestion.countDocuments({
  topic: { $in: [null, ""] },
});
const byPaper = await GmatCompetitiveQuestion.aggregate([
  {
    $group: {
...
const topics = await GmatCompetitiveQuestion.aggregate([
